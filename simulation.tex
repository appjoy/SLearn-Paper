...
\item \slearn significantly improves the JCTs. In simulation using the 2STrace, GTrace11
	and GTrace19, the average JCT is improved by 1.28$\times$ 1.56$\times$ and
	1.32$\times$, respectively, over the prior art \primarybase.
    For 42.54\% , 70.66\% and 60.59\% jobs individual speedup is greater than 
    1 and for 10\% of all the jobs individual speedup is 
    greater than 4.41$\times$, 3.01$\times$ and 12.16$\times$ for the 2STrace, GTrace11
    and GTrace19 respectively.
...
We implemented a proxy scheduler wrapper that plugs into resource
...
omit the sensitivity study here due to page limit.
...
\slearn, assigns the number of pilot tasks assigned to wide jobs using the
adaptive algorithm described in \S\ref{sec:design:namepredict}, and the threshold for thin
jobs is set to \thinLimit. We evaluate the sensitivity of scheduling results to
these parameters in \S\ref{sec:sim:numPilots} and \S\ref{sec:sim:thin}.
...
\subsubsection{Impact of Number of Pilot Tasks}
...
In this experiment we evaluate the effectiveness of our adaptive algorithm for
sampling \S\ref{sec:design:namepredict}. We compare average JCT speedup and P50 speedup
under the adaptive algorithm with those under the fixed sampling percentage
algorithm. We vary the fixed sampling percentage from 1 to 5 and 10. The
adaptive sampling algorithm leads to the best speedups for the 2STrace and
GTrace19 and is only \~1\% worse then the best for
the GTrace11.  However, a key thing to note is that the adaptive algorithm
doesn't provide the best prediction accuracy. This shows that the adaptive
algorithm effectively balances the tradeoff between prediction accuracy and
sampling time. 
